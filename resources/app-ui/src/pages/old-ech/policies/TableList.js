...
import PoliciesScreenListingVideoIframe from "@/components/VideoTutorialsIframes/SuperAdminIframes/PoliciesScreenListingVideoIframe";
...
const TableList = () => {
  ...
  const [policiesListingVideoTutorials, setPoliciesListingVideoTutorials] = useState({});
  const [policiesListingVideoTutorialModelOpen, setPoliciesListingVideoTutorialModelOpen] = useState(false);
  /**
   * End By Mairaj
   */
  ...
  const [policyAutoUpdatesBtnLoading, setPolicyAutoUpdatesBtnLoading] = useState(false);
  ...
  const columns = [
    ...
    },
    {
      title: "Publish Date",
      dataIndex: 'date',
      key: 'filter-column-publish-date',
      hideInTable: true,
      valueType: 'dateRange',
      search: {
        transform: (value) => {
    ...
    },
    {
      ...
      render: (text, record, _, seats) => {
        ...
        console.log('text status case 01')
        console.log(text);
    ...
    {
      title: "Staff Members",
      key: 'table-column-staff-members',
      hideInSearch: true,
      // valueType: 'textarea',
      // render: (record) => {
      //   return (
    ...
    },
    {
      ...
      hideInSearch: true,
      render: (text, record, _, action) => {
        // const auto_updates_status = record?.meta?.ech_auto_updates;
        const auto_updates_status = ( 'yes' === record?.meta?.ech_auto_updates ) ? true: false;
        // const used_seats = record?.meta?.ech_used_seats;
        return (<Switch
            checkedChildren="Enabled"
            unCheckedChildren="Disabled"
            checked={auto_updates_status}
            loading={policyAutoUpdatesBtnLoading}
            onChange={ async ( checked, event ) => {
              console.log('auto-updates - checked');
              console.log(checked);
              console.log('auto-updates - event');
              console.log(event);
              /**
               * API update post meta
               */

              setPolicyAutoUpdatesBtnLoading(true);

                const request_data = {
                  meta: {
                    ech_auto_updates: ( true === checked ) ? 'yes': 'no',
                  },
                };

                // await waitTime(2000);

                 await request('/wp/v2/ech-customer-policy/'+record.id, {
                      method: 'PUT',
                      data: request_data,
                      params: {
                        id: record.id
                ...
                //  .then((api_response) => {
                //
                //    setPolicyAutoUpdatesBtnLoading(false);
                //
                //   /**
                //    * Reload the table after modal closing to show the updated seats in the table
                //    */
      ...
      render: (text, record, _, action) => {
        ...
        let action_buttons = [
          <Space key={'space-row-actions-' + record.id} wrap>
            <Dropdown.Button key={'row-actions-' + record.id} menu={{
              items, onClick: ({key}) => {
                ...
                if (key === 'view') {
                  ...
                  history.push('/policies/view/' + record.id + '/' + organizationId);
                }
                ...
                if (key === 'editable') {
                  ...
                  history.push('/policies/edit/' + record.id + '/' + organizationId);

                }

                if (key === 'policy_versions_link') {
                  console.log('drop-down-menu-item click callback trigred...');
                  console.log('record');
                  console.log(record);

                  history.push('/policies/versions/' + record.id + '/' + record.meta?.ech_parent_policy_id);
          ...
          //   }}
          //   // onClick={() => {
          //   //   console.log("record-testing-for-view");
          //   //   console.log(record);
          //   //
          //   //   setViewSelectedPolicy(record);
          //   //
          //   //   showPolicyOrganizationsDrawer(true);
          //   //
          //   //
          //   //   // history.push(record);
          //   //
         ...
         * Extend Actions
         */
        if( record?.meta?.ech_parent_policy_versions ) {

          const isPendingVersion = record?.meta?.ech_parent_policy_versions.some(parent_policy_version => {
            return parent_policy_version.parent_policy_version_status === 'pending';
          });
    ...
    <PageContainer>
      ...
      >
        <TableTransfer
          ...
          titles={[ 'Not Associated Staff Members', 'Associated Staff Members']}
        />
      </ModalForm>
        ...
        <FloatButton
            ...
            onClick={ ()=> {
              ...
              request('/wp/v2/super-admin-video-tutorials', {
                ...
                const policies_screen_listing_video_tutorials_data = {
                  // ...initialValues,
                  policies_screen: {
                ...
                setPoliciesListingVideoTutorials(policies_screen_listing_video_tutorials_data);

                console.log('policies_screen_listing_video_tutorials_data');
                console.log(policies_screen_listing_video_tutorials_data);

              });
              setPoliciesListingVideoTutorialModelOpen(true);
            }}
        />
        <FloatButton
      ...
      <ModalForm
          title={'Policies Listing Video Tutorial'}
          open={policiesListingVideoTutorialModelOpen}
          onOpenChange={setPoliciesListingVideoTutorialModelOpen}
          modalProps={{
            destroyOnClose: true,
            onCancel: () => console.log('run'),
            afterClose: () => {
      ...
      >
        <VideoTutorialIframe
            iframeUrl={policiesListingVideoTutorials?.policies_screen?.listing_video_url}
        />
