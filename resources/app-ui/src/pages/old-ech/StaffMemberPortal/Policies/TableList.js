...
import PoliciesScreenListingVideoIframe from "@/components/VideoTutorialsIframes/StaffMemberIframes/PoliciesScreenListingVideoIframe";
...
const TableList = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'table-column-acknowledgment',
      // valueType: 'textarea',
      hideInSearch: true,
      render: (text, record, _, seats) => {
        ...
        const policy_user_status_index = initialState?.currentUser?.meta?.ech_user_policies.findIndex(object => {
          return object.policy_id === record?.id
        });

        const user_policy_status = initialState?.currentUser?.meta?.ech_user_policies[policy_user_status_index].policy_status;
    ...
    // },
    {
      ...
      render: (text, record, _, action) => [
        ...
        <Space key={'space-row-actions-' + record.id} wrap>
          <Dropdown.Button key={'row-actions-' + record.id} menu={{items, onClick: ({ key }) => {
              ...
              if (key === 'view') {
                ...
                history.push('/staff-member/policies/view/' + record.id + '/' + organizationId);
              }
        ...
        // <a
        //   key="view"
        //   // href={ () => {('/staff-member/policies/view/' + record.id + '/' + organizationId)}
        //   onClick={() => {
      ...
      <ProTable
        ...
        request={
          ...
          async (params = {}, sort, filter, paginate) => {
            ...
            const user_policies_ids = initialState?.currentUser?.meta?.ech_user_policies.map((user_policy) => user_policy.policy_id);
            return await getEchCustomerPolicies(
              {
                ...
                organization_id: initialState?.currentUser?.meta?.ech_organization_id,
                // include: [438]
                include: (user_policies_ids.length > 0) ? user_policies_ids : [0],
              },
              {getResponse: true}
            );
            // }
