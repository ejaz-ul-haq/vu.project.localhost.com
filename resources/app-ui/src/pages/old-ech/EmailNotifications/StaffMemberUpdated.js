...
import EmailNotificationsScreenStaffMemberUpdatedVideoIframe from "@/components/VideoTutorialsIframes/SuperAdminIframes/EmailNotificationsScreenStaffMemberUpdatedVideoIframe";
import StaffMemberUpdatedSkeleton from "@/components/Skeleton/EmailNotifications/StaffMemberUpdatedSkeleton";
...
const StaffMemberUpdated = ( { status, subject, body } ) => {
  ...
  const [emailNotificationId, setEmailNotificationId] = useState('staff_member_updated');
  const [staffMemberUpdatedData, setStaffMemberUpdatedData] = useState({});
  const [staffMemberUpdatedSkeleton, setStaffMemberUpdatedSkeleton] = useState(true);
  ...
  const [emailNotificationsStaffMemberUpdatedVideoTutorials, setEmailNotificationsStaffMemberUpdatedVideoTutorials] = useState({});
  const [emailNotificationsStaffMemberUpdatedVideoTutorialModelOpen, setEmailNotificationsStaffMemberUpdatedVideoTutorialModelOpen] = useState(false);
  /**
   * End By Mairaj
   */
  ...
  useEffect(() => {
    ...
    request('/wp/v2/email-notification/?organization_id='+organizationId+'&email_notification_id='+emailNotificationId, {
      ...
      setStaffMemberUpdatedData(api_response);

      setStaffMemberUpdatedSkeleton(false);
   ...
   */
  useEffect(() => {
    form.setFieldValue('status', staffMemberUpdatedData?.status);
    form.setFieldValue('subject', staffMemberUpdatedData?.subject);
    form.setFieldValue('body', staffMemberUpdatedData?.body);

    if (staffMemberUpdatedData?.status === true) {
  ...
  //
  //
  // console.log('staff-member-updated - subject');
  // console.log(subject);
        ...
        <PageContainer>
          <ProForm
              ...
              onFinish={async (values) => {
                ...
                values.email_notification_id = 'staff_member_updated';
            ...
            <FloatButton
                ...
                onClick={() => {
                  ...
                  request('/wp/v2/super-admin-video-tutorials', {
                    ...
                    const email_notifications_screen_staff_member_updated_video_tutorials_data = {
                      ...initialValues,
                    ...
                    setEmailNotificationsStaffMemberUpdatedVideoTutorials(email_notifications_screen_staff_member_updated_video_tutorials_data);

                    console.log('email_notifications_screen_staff_member_updated_video_tutorials_data');
                    console.log(email_notifications_screen_staff_member_updated_video_tutorials_data);

                  });
                  setEmailNotificationsStaffMemberUpdatedVideoTutorialModelOpen(true);
                }}
            />
            <FloatButton
          ...
          <ModalForm
              title={'Email Notifications Staff Member Updated Video Tutorial'}
              open={emailNotificationsStaffMemberUpdatedVideoTutorialModelOpen}
              onOpenChange={setEmailNotificationsStaffMemberUpdatedVideoTutorialModelOpen}
              modalProps={{
                destroyOnClose: true,
          ...
          >
            <VideoTutorialIframe
                iframeUrl={emailNotificationsStaffMemberUpdatedVideoTutorials?.email_notifications_screen?.staff_member_updated_video_url}
            />
...
export default StaffMemberUpdated;
...
//
//
// export default React.memo(StaffMemberUpdated, areEqual)
