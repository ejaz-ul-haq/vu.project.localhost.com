...
import EmailNotificationsScreenSuperUserCreatedVideoIframe from "@/components/VideoTutorialsIframes/SuperAdminIframes/EmailNotificationsScreenSuperUserCreatedVideoIframe";
import SuperUserCreatedSkeleton from "@/components/Skeleton/EmailNotifications/SuperUserCreatedSkeleton";
...
const SuperUserCreated = ( { status, subject, body } ) => {
    ...
    const [emailNotificationId, setEmailNotificationId] = useState('super_user_created');
    const [superUserCreatedData, setSuperUserCreatedData] = useState({});
    const [superUserCreatedSkeleton, setSuperUserCreatedSkeleton] = useState(true);
    ...
    const [emailNotificationsSuperUserCreatedVideoTutorials, setEmailNotificationsSuperUserCreatedVideoTutorials] = useState({});
    const [emailNotificationsSuperUserCreatedVideoTutorialModelOpen, setEmailNotificationsSuperUserCreatedVideoTutorialModelOpen] = useState(false);
    /**
     * End By Mairaj
     */
  ...
  useEffect(() => {
    ...
    useEffect(() => {
        ...
        request('/wp/v2/email-notification/?organization_id='+organizationId+'&email_notification_id='+emailNotificationId, {
            ...
            setSuperUserCreatedData(api_response);

            setSuperUserCreatedSkeleton(false);
     ...
     */
    useEffect(() => {
        form.setFieldValue('status', superUserCreatedData?.status);
        form.setFieldValue('subject', superUserCreatedData?.subject);
        form.setFieldValue('body', superUserCreatedData?.body);

        if (superUserCreatedData?.status === true) {
            ...
            <PageContainer>
                <ProForm
                    ...
                    onFinish={async (values) => {
                        ...
                        values.email_notification_id = 'super_user_created';
                    ...
                    <FloatButton
                        ...
                        onClick={() => {
                            ...
                            request('/wp/v2/super-admin-video-tutorials', {
                                ...
                                const email_notifications_screen_super_user_created_video_tutorials_data = {
                                    ...initialValues,
                                ...
                                setEmailNotificationsSuperUserCreatedVideoTutorials(email_notifications_screen_super_user_created_video_tutorials_data);

                                console.log('email_notifications_screen_super_user_created_video_tutorials_data');
                                console.log(email_notifications_screen_super_user_created_video_tutorials_data);

                            });
                            setEmailNotificationsSuperUserCreatedVideoTutorialModelOpen(true);
                        }}
                    />
                    <FloatButton
                ...
                <ModalForm
                    title={'Email Notifications Super User Created Video Tutorial'}
                    open={emailNotificationsSuperUserCreatedVideoTutorialModelOpen}
                    onOpenChange={setEmailNotificationsSuperUserCreatedVideoTutorialModelOpen}
                    modalProps={{
                        destroyOnClose: true,
                ...
                >
                    <VideoTutorialIframe
                        iframeUrl={emailNotificationsSuperUserCreatedVideoTutorials?.email_notifications_screen?.super_user_created_video_url}
                    />
...
export default SuperUserCreated;
...
//
//
// export default React.memo(SuperUserCreated, areEqual)
